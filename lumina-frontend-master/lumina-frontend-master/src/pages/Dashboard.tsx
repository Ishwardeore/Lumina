...
const Dashboard = () => {
    ...
    const handleScheduleRound = async () => {
        if (!scheduleRoundData || !newRound.scheduledDate) {
            toast.error('Please select a date and time for the round');
            return;
        }
        ...
        try {
            const response = await reminderService.createReminder({
                jobId: scheduleRoundData.job.id,
                company: scheduleRoundData.job.company,
                role: scheduleRoundData.job.role,
            ...
            if (response.emailConfigured) {
                ...
                toast.warning('Round scheduled, but SMTP email is not configured on the server.');
            }
                        ...
                        >
                            ...
                            >
                                ...
                                <div className="space-y-4 mb-6">
                                    {/* Scheduled Date */}
                                    <div>
                                        <label className="block text-xs font-bold text-gray-700 mb-2">Scheduled Date & Time</label>
                                        <input
                                            type="datetime-local"
