...
import { resumeService, MatchScoreResult } from '../services/resumeService'; // Service Layer
import { authService } from '../services/authService';
import { Button } from '../components/ui/Button';
// import { TextArea } from '../components/ui/TextArea';
import { Card } from '../components/ui/Card';
import { ArrowLeft, FileText, Upload, Plus, Briefcase, Wand2, Target, CheckCircle2, AlertTriangle, Sparkles, Save, PencilLine } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { EXPERIENCE_LEVELS, STORAGE_KEYS, TEMPLATES } from '../utils/constants';
...
const resumeToText = (resume: ResumeData) => [
    resume.summary,
    ...(resume.skills || []),
    ...(resume.experience || []).map(item => `${item.title} ${item.company} ${item.description}`),
    ...(resume.projects || []).map(item => `${item.name} ${item.tech} ${item.description}`),
    ...(resume.education || []).map(item => `${item.degree} ${item.school}`),
    ...(resume.certifications || []).map(item => `${item.name} ${item.issuer}`)
].filter(Boolean).join(' ');
...
const Generator: React.FC = () => {
    ...
    const [activeTab, setActiveTab] = useState<'import' | 'generate' | 'match'>('import');
    ...
    const [template, setTemplate] = useState<string>(TEMPLATES.DETAILED);
    const [generateLoading, setGenerateLoading] = useState(false);
    const [saveGeneratedLoading, setSaveGeneratedLoading] = useState(false);
    const [generatedResume, setGeneratedResume] = useState<ResumeData | null>(null);
    const [generatedMatch, setGeneratedMatch] = useState<MatchScoreResult | null>(null);
    ...
    */
    ...
    const handleMatchScore = async () => {
        if (matchResumeText.trim().length < 80) {
            toast.error("Paste more resume content for an accurate score.");
            return;
        }

        if (matchJobDescription.trim().length < 80) {
            toast.error("Paste the full job description for an accurate score.");
            return;
        }

        setMatchLoading(true);
        try {
            const result = await resumeService.analyzeMatchScore({
                resumeText: matchResumeText,
                jobDescription: matchJobDescription
            });
            setMatchResult(result);
            toast.success("Resume match score ready.");
        } catch (err: any) {
            const errorMsg = err.response?.data?.message || "Failed to analyze resume match.";
    ...
    const handleGenerateResume = async () => {
        if (jd.trim().length < 80) {
            toast.error("Paste a fuller job description before generating.");
            return;
        }
        ...
        try {
            const resume = await resumeService.generateResume({ jd, level, template });
            setGeneratedResume(resume);
            setResumeData(resume);
            localStorage.setItem(STORAGE_KEYS.RESUME_DATA, JSON.stringify(resume));

            try {
                const match = await resumeService.analyzeMatchScore({
                    resumeText: resumeToText(resume),
                    jobDescription: jd
                });
                setGeneratedMatch(match);
            } catch (matchError) {
                console.warn("Generated resume match score failed", matchError);
            }

            toast.success("AI resume generated.");
        } catch (err: any) {
            const errorMsg = err.response?.data?.error || err.response?.data?.message || "Failed to generate resume.";
            toast.error(errorMsg);
        } finally {
    ...
    const handleOpenGeneratedInEditor = () => {
        if (!generatedResume) return;
        setResumeData(generatedResume);
        localStorage.setItem(STORAGE_KEYS.RESUME_DATA, JSON.stringify(generatedResume));
        navigate('/editor', { state: { template, viewMode: 'split' } });
    };

    const handleSaveGeneratedResume = async () => {
        if (!generatedResume) return;

        setSaveGeneratedLoading(true);
        try {
            const titleName = generatedResume.personalInfo?.fullName || 'Generated';
            const payload = {
                title: `${titleName} - AI Resume`,
                content: generatedResume,
                template,
                updatedAt: new Date().toISOString()
            };

            const response = await resumeService.saveResume(payload);
            const savedResume = response?.resume ? { ...response.resume, content: generatedResume } : payload;
            const savedResumes = localStorage.getItem('savedResumes');
            const resumes = savedResumes ? JSON.parse(savedResumes) : [];
            resumes.unshift(savedResume);
            localStorage.setItem('savedResumes', JSON.stringify(resumes));

            toast.success("Generated resume saved.");
        } catch (err: any) {
            const errorMsg = err.response?.data?.message || "Failed to save generated resume.";
            toast.error(errorMsg);
        } finally {
            setSaveGeneratedLoading(false);
        }
    };
                        ...
                        <Card className="relative border border-white/50 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.05),0_0_0_1px_rgba(255,255,255,0.5)] backdrop-blur-xl bg-white/95 rounded-[1.5rem] p-6 overflow-hidden">
                            ...
                            <div className="bg-gray-100/50 p-1 rounded-full mb-4 relative">
                                <div className="grid grid-cols-3 relative z-10">
                                    ...
                                    <button
                                        ...
                                        className={`relative flex items-center justify-center gap-2 py-2.5 text-sm font-semibold rounded-full transition-all duration-300 ${activeTab === 'match' ? 'text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                                    >
                                        {activeTab === 'match' && (
                                            <motion.div
                                                layoutId="activeTab"
                                    ...
                                    <button
                                        ...
                                        className={`relative flex items-center justify-center gap-2 py-2.5 text-sm font-semibold rounded-full transition-all duration-300 ${activeTab === 'generate' ? 'text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                                    >
                                        {activeTab === 'generate' && (
                                            <motion.div
                                                layoutId="activeTab"
                            ...
                            {activeTab === 'import' ? (
                                ...
                                <motion.div
                                    ...
                                    className="space-y-4"
                                >
                                    <div className="bg-blue-50/40 border-l-2 border-blue-400/60 rounded-r-xl p-3 flex items-center gap-4">
                                        <div className="w-8 h-8 rounded-full bg-blue-50 flex items-center justify-center flex-shrink-0">
                                            <Target className="text-blue-600" size={16} />
                                        </div>
                                        <div>
                                            <h3 className="text-sm font-semibold text-gray-800">Resume Match Score</h3>
                                            <p className="text-xs text-gray-500 mt-0.5">Paste your resume and a job description to find gaps before applying.</p>
                                        </div>
                                    </div>

                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="space-y-1">
                                            <label className="text-xs font-medium text-gray-600 ml-1">Resume Text</label>
                                            <textarea
                                                ...
                                                className="w-full min-h-[170px] resize-none p-3 bg-white border border-gray-200 rounded-xl text-sm outline-none focus:border-blue-500/50 focus:shadow-[0_0_0_4px_rgba(59,130,246,0.1)] transition-all placeholder:text-gray-300"
                                            />
                                            <p className="text-[10px] text-gray-400 ml-1">{matchResumeText.trim().length} characters</p>
                                        </div>

                                        <div className="space-y-1">
                                            <label className="text-xs font-medium text-gray-600 ml-1">Job Description</label>
                                            <textarea
                                                ...
                                                className="w-full min-h-[170px] resize-none p-3 bg-white border border-gray-200 rounded-xl text-sm outline-none focus:border-blue-500/50 focus:shadow-[0_0_0_4px_rgba(59,130,246,0.1)] transition-all placeholder:text-gray-300"
                                            />
                                            <p className="text-[10px] text-gray-400 ml-1">{matchJobDescription.trim().length} characters</p>
                                        </div>
                                    </div>
                                    ...
                                    <Button
                                        ...
                                        className="w-full relative flex items-center justify-center py-3 text-sm font-bold tracking-wide shadow-xl shadow-blue-500/20 rounded-xl bg-gradient-to-t from-blue-700 to-blue-600 hover:from-blue-600 hover:to-blue-500 hover:shadow-blue-500/40 hover:-translate-y-0.5 active:translate-y-0 active:shadow-sm transition-all duration-300 disabled:opacity-70 disabled:hover:translate-y-0"
                                    >
                                        {matchLoading ? <Wand2 className="mr-2 animate-spin" size={16} /> : <Target size={18} strokeWidth={3} className="mr-2" />}
                                        {matchLoading ? 'Analyzing...' : 'Analyze Match Score'}
                                    </Button>
                                    ...
                                    {matchResult && (
                                        <motion.div
                                            ...
                                            animate={{ opacity: 1, y: 0 }}
                                            className="border border-gray-100 rounded-2xl bg-white/80 p-4 space-y-4"
                                        >
                                            <div className="flex flex-col sm:flex-row sm:items-center gap-4 justify-between">
                                                <div className="flex items-center gap-4">
                                                    <div className={`w-20 h-20 rounded-full flex items-center justify-center text-2xl font-black border-4 ${matchResult.score >= 80 ? 'bg-green-50 text-green-700 border-green-100' : matchResult.score >= 60 ? 'bg-blue-50 text-blue-700 border-blue-100' : 'bg-orange-50 text-orange-700 border-orange-100'}`}>
                                                        {matchResult.score}
                                                    </div>
                                                    <div>
                                                        <p className="text-xs uppercase tracking-wide font-bold text-gray-400">ATS-style match</p>
                                                        <h3 className="text-xl font-bold text-gray-900">{matchResult.verdict}</h3>
                                                        <p className="text-sm text-gray-500">Improve missing keywords honestly before applying.</p>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-3 gap-2">
                                                {([
                                                    ['Keywords', matchResult.breakdown.keywordCoverage],
                                                    ['Depth', matchResult.breakdown.resumeDepth],
                                                    ['Metrics', matchResult.breakdown.measurableImpact]
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                <div>
                                                    <div className="flex items-center gap-2 mb-2">
                                                        ...
                                                        <h4 className="text-sm font-bold text-gray-800">Matched Keywords</h4>
                                                    </div>
                                                    <div className="flex flex-wrap gap-2">
                                                        {matchResult.matchedKeywords.length ? matchResult.matchedKeywords.map(keyword => (
                                                            <span key={keyword} className="px-2.5 py-1 rounded-full bg-green-50 text-green-700 border border-green-100 text-xs font-semibold">{keyword}</span>
                                                        )) : <p className="text-xs text-gray-400">No strong keyword matches yet.</p>}
                                                    </div>
                                                </div>
                                                ...
                                                <div>
                                                    <div className="flex items-center gap-2 mb-2">
                                                        ...
                                                        <h4 className="text-sm font-bold text-gray-800">Missing Keywords</h4>
                                                    </div>
                                                    <div className="flex flex-wrap gap-2">
                                                        {matchResult.missingKeywords.map(keyword => (
                                                            <span key={keyword} className="px-2.5 py-1 rounded-full bg-orange-50 text-orange-700 border border-orange-100 text-xs font-semibold">{keyword}</span>
                                                        ))}
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="space-y-2">
                                                <h4 className="text-sm font-bold text-gray-800">Suggested Improvements</h4>
                                                {matchResult.suggestions.map((suggestion) => (
                                                    <div key={suggestion} className="flex items-start gap-2 text-sm text-gray-600">
                                ...
                                <motion.div
                                    ...
                                    className="space-y-4"
                                >
                                    <div className="bg-indigo-50/40 border-l-2 border-indigo-400/60 rounded-r-xl p-3 flex items-center gap-4">
                                        <div className="w-8 h-8 rounded-full bg-indigo-50 flex items-center justify-center flex-shrink-0">
                                            <Sparkles className="text-indigo-600" size={16} />
                                        </div>
                                        <div>
                                            <h3 className="text-sm font-semibold text-gray-800">Generate with AI</h3>
                                            <p className="text-xs text-gray-500 mt-0.5">Create a job-specific resume draft from a full job description.</p>
                                        </div>
                                    </div>

                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="space-y-1">
                                            <label className="text-xs font-medium text-gray-600 ml-1">Experience Level</label>
                                            <select
                                                value={level}
                                                onChange={(e) => setLevel(e.target.value)}
                                                className="w-full p-2.5 bg-white border border-gray-200 rounded-xl text-sm outline-none focus:border-blue-500/50 focus:shadow-[0_0_0_4px_rgba(59,130,246,0.1)] transition-all"
                                            >
                                                {EXPERIENCE_LEVELS.map(item => (
                                                    <option key={item.id} value={item.id}>{item.label} ({item.years})</option>
                                                ))}
                                            </select>
                                        </div>

                                        <div className="space-y-1">
                                            <label className="text-xs font-medium text-gray-600 ml-1">Template Style</label>
                                            <div className="grid grid-cols-2 gap-2">
                                                {[
                                                    { id: TEMPLATES.DETAILED, label: 'Detailed' },
                                                    { id: TEMPLATES.COMPACT, label: 'Compact' }
                                                ].map(item => (
                                                    <button
                                                        key={item.id}
                                                        type="button"
                                                        onClick={() => setTemplate(item.id)}
                                                        className={`rounded-xl border px-3 py-2.5 text-sm font-semibold transition-all ${template === item.id ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 bg-white text-gray-500 hover:border-gray-300'}`}
                                                    >
                                                        {item.label}
                                                    </button>
                                                ))}
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <textarea
                                            ...
                                            placeholder="Paste the full job description. Include responsibilities, required skills, tools, and experience expectations..."
                                            className="w-full min-h-[210px] resize-none p-3 bg-white border border-gray-200 rounded-xl text-sm outline-none focus:border-blue-500/50 focus:shadow-[0_0_0_4px_rgba(59,130,246,0.1)] transition-all placeholder:text-gray-300"
                                        />
                                        <p className="text-[10px] text-gray-400 ml-1">{jd.trim().length} characters</p>
                                    </div>

                                    <Button
                                        onClick={handleGenerateResume}
                                        disabled={generateLoading}
                                        className="w-full relative flex items-center justify-center py-3 text-sm font-bold tracking-wide shadow-xl shadow-indigo-500/20 rounded-xl bg-gradient-to-t from-indigo-700 to-blue-600 hover:from-indigo-600 hover:to-blue-500 hover:shadow-indigo-500/40 hover:-translate-y-0.5 active:translate-y-0 active:shadow-sm transition-all duration-300 disabled:opacity-70 disabled:hover:translate-y-0"
                                    >
                                        {generateLoading ? <Wand2 className="mr-2 animate-spin" size={16} /> : <Sparkles size={18} strokeWidth={3} className="mr-2" />}
                                        {generateLoading ? 'Generating...' : 'Generate AI Resume'}
                                    </Button>
                                    ...
                                    {generatedResume && (
                                        <motion.div
                                            initial={{ opacity: 0, y: 12 }}
                                            animate={{ opacity: 1, y: 0 }}
                                            className="border border-gray-100 rounded-2xl bg-white/90 p-4 space-y-4"
                                        >
                                            <div className="flex flex-col md:flex-row gap-3 md:items-center md:justify-between">
                                                <div>
                                                    <p className="text-xs uppercase tracking-wide font-bold text-gray-400">Generated Draft</p>
                                                    <h3 className="text-xl font-bold text-gray-900">{generatedResume.personalInfo?.fullName || 'AI Resume'}</h3>
                                                    <p className="text-sm text-gray-500">{generatedResume.skills?.slice(0, 6).join(', ')}</p>
                                                </div>
                                                {generatedMatch && (
                                                    <div className={`w-16 h-16 rounded-full flex items-center justify-center text-xl font-black border-4 ${generatedMatch.score >= 80 ? 'bg-green-50 text-green-700 border-green-100' : generatedMatch.score >= 60 ? 'bg-blue-50 text-blue-700 border-blue-100' : 'bg-orange-50 text-orange-700 border-orange-100'}`}>
                                                        {generatedMatch.score}
                                                    </div>
                                                )}
                                            </div>
                                            ...
                                            <div className="space-y-3">
                                                <section>
                                                    <h4 className="text-sm font-bold text-gray-800 mb-1">Summary</h4>
                                                    <p className="text-sm text-gray-600 leading-relaxed">{generatedResume.summary}</p>
                                                </section>

                                                {generatedResume.projects?.length > 0 && (
                                                    <section>
                                                        <h4 className="text-sm font-bold text-gray-800 mb-2">Projects</h4>
                                                        <div className="space-y-2">
                                                            {generatedResume.projects.slice(0, 2).map((project, index) => (
                                                                <div key={`${project.name}-${index}`} className="rounded-xl bg-gray-50 border border-gray-100 p-3">
                                                ...
                                                {generatedMatch && generatedMatch.missingKeywords.length > 0 && (
                                                    <section>
                                                        <h4 className="text-sm font-bold text-gray-800 mb-2">Remaining Missing Keywords</h4>
                                                        <div className="flex flex-wrap gap-2">
                                                            {generatedMatch.missingKeywords.slice(0, 10).map(keyword => (
                                                                <span key={keyword} className="px-2.5 py-1 rounded-full bg-orange-50 text-orange-700 border border-orange-100 text-xs font-semibold">{keyword}</span>
                                                            ))}
                                                        </div>
                                                    </section>
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                <Button
                                                    ...
                                                    className="flex items-center justify-center gap-2 py-3 rounded-xl bg-slate-900 hover:bg-slate-800"
                                                >
                                                    {saveGeneratedLoading ? <Wand2 size={16} className="animate-spin" /> : <Save size={16} />}
                                                    {saveGeneratedLoading ? 'Saving...' : 'Save Resume'}
                                                </Button>
                                                <Button
                                                    onClick={handleOpenGeneratedInEditor}
                                                    className="flex items-center justify-center gap-2 py-3 rounded-xl bg-blue-600 hover:bg-blue-500"
                                                >
                                                    <PencilLine size={16} />
                                                    Edit Resume
                                                </Button>
